...
p_best_val = arrayfun(@(i) fitness(x(i, 1), x(i, 2)), 1:pop_size);% 计算初始适应度
[g_best_val, g_best_idx] = max(p_best_val); 
...
for iter = 1:max_iter
    ...
    [current_best_val, current_best_idx] = max(p_best_val);     % 更新全局最佳
    ...
    fprintf('第%d次迭代: 最优值 = %.4f\n', iter, g_best_val);     % 输出迭代信息
end


fprintf('最优解: x1 = %.4f, x2 = %.4f\n', g_best(1), g_best(2)); % 输出结果
fprintf('最优值: %.4f\n', g_best_val);
